...
export const getSenderImage = (loggedInUser, users) => {
  return loggedInUser === users[0]._id
    ? users[1]?.profile_img?.url
    : users[0]?.profile_img?.url;
};
...
export const isUserBlocked = (users, loggedInUser) => {
  const user = users[0]._id === loggedInUser ? users[0] : users[1];
  const chatter = users[0]._id === loggedInUser ? users[1] : users[0]; 

    if(user?.blockList?.includes(chatter._id)){
...
export const isChatBlocked = (users, loggedInUser) => {
    const user = users[0]._id === loggedInUser ? users[0] : users[1];
  const chatter = users[0]._id === loggedInUser ? users[1] : users[0]; 

    if(chatter?.blockList?.includes(user._id)){
...
export const receiver = (users, loggedInUser) =>{
    return users[0]._id === loggedInUser?users[1]._id:users[0]._id
}
